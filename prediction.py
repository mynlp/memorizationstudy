...
class BayesianLSTM(PyroModule):
    def __init__(self, input_size=1, hidden_size=50, out_size=1):
        ...
        self.lstm.weight_ih_l0 = PyroSample(dist.Normal( torch.tensor(0, device="cuda"),  torch.tensor(1.0, device="cuda")).expand([4 * hidden_size, input_size]).to_event(2))
        self.lstm.weight_hh_l0 = PyroSample(dist.Normal(torch.tensor(0, device="cuda"),  torch.tensor(1.0, device="cuda")).expand([4 * hidden_size, hidden_size]).to_event(2))
        self.linear = PyroModule[nn.Linear](hidden_size, out_size)
        self.linear.weight = PyroSample(dist.Normal(torch.tensor(0, device="cuda"),  torch.tensor(1.0, device="cuda")).expand([out_size, hidden_size]).to_event(2))
        self.cuda()
