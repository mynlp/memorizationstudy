...
for model_size in ["410m", "2.8b", "12b"]:
    ...
    for token in range(2, continuation+1):
        ...
        sns.heatmap(similarities.astype(float), annot=True, fmt=".3f", annot_kws={"size": 7},
                    square=True, cmap='hot', xticklabels=True, yticklabels=True, ax=axs[0])

        axs[0].set_title(f'Embedding Similarities_{token}')
        ...
        sns.heatmap(distances.astype(float), annot=True, fmt=".2f", annot_kws={"size": 9},
                    square=True, cmap=cmap, xticklabels=True, yticklabels=True, ax=axs[1])

        axs[1].set_title(f'Embedding Distances_{token}')

        plt.tight_layout()  # adjust subplot params so that subplots fit into the figure area
        plt.savefig(f'embedding_figure/embedding_{model_size}_{token}.png', bbox_inches='tight', pad_inches=0)
        plt.show()
