...
# last hidden state
context_embedding = generations_full_memo.hidden_states[0][-1].mean(1).squeeze().cpu().numpy()
context_embedding_not_full = generations_not_full.hidden_states[0][-1].mean(1).squeeze().cpu().numpy()
context_embedding_half_memo = generations_half_memo.hidden_states[0][-1].mean(1).squeeze().squeeze().cpu().numpy()
for token in range(2, 16):
    embedding = torch.stack([x[-1] for x in generations_full_memo.hidden_states[1:token]]).mean(0).squeeze().cpu().numpy()
    embedding_not_full = torch.stack([x[-1] for x in generations_not_full.hidden_states[1:token]]).mean(0).squeeze().cpu().numpy()
    embedding_half_memo = torch.stack([x[-1] for x in generations_half_memo.hidden_states[1:token]]).mean(0).squeeze().cpu().numpy()
